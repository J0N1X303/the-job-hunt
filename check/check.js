(()=>{
  ...
  function renderWelcome(){
    ...
    $('#welcomeText').textContent=ago===0?'You already checked in today. You can run it again, but weekly changes will be more meaningful.':`Your last saved check was ${ago} day${ago===1?'':'s'} ago. Run it again to see what changed.`;
  }
  ...
  function diagnose(){
    ...
    let title,explain,signals,donts,plan,confidence='Medium confidence';
    ...
    return {date:today(),role,level,weeks,breadth,apps,convs,ints,finals,offers,warm,fit,tailor,mot,reach,intRate,finalRate,offerRate,title,explain,signals,donts,plan,confidence};
  }
  ...
  function renderResult(r){
    ...
    if(prev){
      ...
      $('#previousDiagnosis').textContent=prev.title?`Last diagnosis: ${prev.title}`:'';
    }else compare.classList.add('hidden');

    $('#saveStatus').textContent='Not saved yet. Saving keeps this result only in this browser.';
    $('#saveBtn').disabled=false;
    window.scrollTo({top:0,behavior:'smooth'});
    track('check_complete',{diagnosis:r.title,apps:r.apps,conversations:r.convs,interviews:r.ints,finals:r.finals,motivation:r.mot,returning:Boolean(prev)});
  }
  ...
  $('#saveBtn').addEventListener('click',()=>{
    ...
    const compact={date:currentResult.date,role:currentResult.role,apps:currentResult.apps,convs:currentResult.convs,ints:currentResult.ints,finals:currentResult.finals,offers:currentResult.offers,mot:currentResult.mot,title:currentResult.title};
    if(sameDay>=0)h[sameDay]=compact;else h.push(compact);
    if(saveHistory(h)){
      $('#saveStatus').textContent='Saved on this device. Come back next week and CV Chaos will compare the change.';
      $('#saveBtn').disabled=true;
      track('check_saved',{diagnosis:currentResult.title,history_count:h.length});
      renderWelcome();
  ...
  $('#feedbackBtn').addEventListener('click',()=>{
    ...
    const comment=$('#feedbackText').value.trim().slice(0,400);
    if(!changed||!useful){$('#feedbackStatus').textContent='Please choose the first two options so the feedback is useful.';return}
    ...
    try{
      window.umami.track('check_feedback',{changed_plan:changed,useful,comment:comment||'none',diagnosis:currentResult?.title||'unknown',returning:history().length>0});
      $('#feedbackStatus').textContent='Thank you — feedback sent. No name or email was requested.';
      $('#feedbackBtn').disabled=true;
    }catch(_){$('#feedbackStatus').textContent='Feedback could not send this time.'}
  });

  const params=new URLSearchParams(location.search);
  track('check_view',{source:params.get('utm_source')||'direct',medium:params.get('utm_medium')||'none',campaign:params.get('utm_campaign')||'none',returning:history().length>0});
  $('#startBtn').addEventListener('click',()=>{track('check_start',{returning:history().length>0});$('#intro').classList.add('hidden');$('#wizard').classList.remove('hidden');showStep(0)});
  $('#welcomeStart')?.addEventListener('click',()=>{track('check_start',{returning:true});$('#intro').classList.add('hidden');$('#wizard').classList.remove('hidden');showStep(0)});
  renderWelcome();
})();