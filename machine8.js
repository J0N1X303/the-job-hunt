(()=>{
  ...
  let shortlistProgress=0,shortlisted=0;
  const shortlistTarget=8;
  let cvs=[],obstacles=[],powerups=[],particles=[],floaters=[];
  let introUntil=0,recruiterUntil=0,recruiterGateId=0,recruiterGapX=0,boostUntil=0,nextPowerAt=0,nextChaosAt=0,bigT=0;
  ...
  function reset(){
    elapsed=0;aimX=W/2;lastAimX=aimX;aimVelocity=0;steady=.15;shotClock=0;interviews=sent=shortlisted=shortlistProgress=0;idleTime=0;motivation=74;
  ...
  function drawPower(p,now){
    ...
    ctx.shadowColor='#7cf3ad';ctx.shadowBlur=15;ctx.fillStyle='rgba(8,32,26,.98)';ctx.strokeStyle='#7cf3ad';ctx.lineWidth=2;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.shadowBlur=0;ctx.fillStyle='#7cf3ad';ctx.textAlign='center';ctx.font='900 9px system-ui';ctx.fillText(p.kind==='INTRO'?'INTRO':'RECRUITER',p.x,p.y+3);ctx.restore();
  }
  ...
  function endGame(){
    ...
    $('resultLine').textContent=`${shortlisted} CVs reached a hiring team. ${interviews} converted to interview${interviews===1?'':'s'}. You finished on ${Math.round(motivation)}% motivation.`;draw(performance.now());
  }
