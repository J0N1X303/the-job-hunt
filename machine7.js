(()=>{
  ...
  function reset(){
    elapsed=0;aimX=W/2;lastAimX=aimX;aimVelocity=0;steady=.15;shotClock=0;interviews=sent=combo=bestCombo=0;idleTime=0;motivation=74;
  ...
  function target(){
    const top=obstacles[obstacles.length-1],widths=[148,132,118,106];return{x:top?top.gapX:routeX,width:widths[phase()],y:Math.max(154,H*.19),h:42};
  }
  ...
  function scoreInterview(cv,y){
    interviews++;combo++;bestCombo=Math.max(bestCombo,combo);interviewBoost();burst(cv.x,y,'#d9ff69',18);boom(combo>1?'INTERVIEW ×'+combo:'INTERVIEW!');cv.alive=false;
  }
  ...
  function update(dt,now){
    ...
    for(const cv of cvs){
      ...
      if(cv.y<=tar.y+tar.h/2){
        ...
        else{cv.alive=false;combo=0;if(elapsed>=nextSetbackAt){motivationChange(-2.2,'MISSED');nextSetbackAt=elapsed+1.5}floaters.push({t:'MISSED',x:cv.x,y:tar.y+24,l:.45,color:'#8191a6'})}
      }
    }
  ...
  function drawGate(o,now){
    ...
    if(o.type==='AI SCREEN'){
      ...
      ctx.fillStyle='rgba(174,140,255,.32)';ctx.fillRect(scan-2,o.y-13,4,26);
      ctx.strokeStyle='rgba(174,140,255,.45)';ctx.beginPath();ctx.moveTo(scan-10,o.y-13);ctx.lineTo(scan+10,o.y-13);ctx.stroke();
    }
    ctx.restore();
  }
