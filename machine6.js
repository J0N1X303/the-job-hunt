(()=>{
  ...
  let aimX=0,lastAimX=0,aimVelocity=0,steady=0,shotClock=0,interviews=0,sent=0,combo=0,bestCombo=0,seq=0,idleTime=0,motivation=70;
  ...
  function motivationChange(amount,label){
    ...
    if(label&&Math.abs(motivation-before)>1.5)floaters.push({t:label,x:W*.5,y:H*.83,l:.75,color:amount>0?'#d9ff69':'#ff9aa6'});
  }
  ...
  function reset(){
    elapsed=0;aimX=W/2;lastAimX=aimX;aimVelocity=0;steady=.15;shotClock=0;interviews=sent=combo=bestCombo=0;idleTime=0;motivation=70;
    cvs=[];powerups=[];particles=[];floaters=[];introUntil=recruiterUntil=boostUntil=0;recruiterGateId=0;recruiterGapX=W/2;
    nextPowerAt=3.8;nextChaosAt=4.7;routeX=W/2;routeTarget=W*.73;routeEase=1.6;nextRouteAt=1.2;resetObstacles();updateHud();
  }
  ...
  function spawnCV(){
    const focus=clamp(steady/1.15,0,1),m=motivation/100;
    const spread=32-(25*focus);
    const interval=(.22-.12*m)+(.07*focus);
  ...
  function target(){
    const top=obstacles[obstacles.length-1],widths=[148,132,118,106];
  ...
  function changeChaos(){
    ...
    o.type=pick(blockers);o.gapW=Math.max(82,W*widths[p]);o.offset=clamp(o.offset+rnd(-16,16),-28,28);o.wander=clamp(o.wander+rnd(-4,6),8,26);o.lag=clamp(o.lag+rnd(-.3,.35),1.45,3.1);o.flash=1;
  ...
  function hitPower(p,cv,now){
    p.life=0;burst(p.x,p.y,'#7cf3ad',24);cv.powerLock=0;
    if(p.kind==='INTRO'){
      introUntil=now+4700;boostUntil=introUntil;motivationChange(7,'MOTIVATION +7');boom('NETWORK INTRO');
      floaters.push({t:'NEXT GAPS OPEN',x:p.x,y:p.y,l:1.1,color:'#7cf3ad'});
    }else{
      recruiterUntil=now+4300;boostUntil=recruiterUntil;motivationChange(9,'MOTIVATION +9');
      const choices=obstacles.slice(1);const gate=pick(choices.length?choices:obstacles);recruiterGateId=gate.id;recruiterGapX=clamp(p.x,52,W-52);boom('GOOD RECRUITER');
      floaters.push({t:'SHORTCUT OPEN',x:p.x,y:p.y,l:1.1,color:'#7cf3ad'});
    }
  }
  ...
  function obstacleCollision(cv,o,now){
    ...
    if(o.type==='500+'&&Math.random()<.55){cv.vx+=rnd(-105,105);cv.vy*=.72;floaters.push({t:'500+',x:cv.x,y:cv.y,l:.45,color:'#ffca72'});motivationChange(-.03);return false}
    cv.alive=false;motivationChange(-.045);burst(cv.x,cv.y,'#ff7586',7);floaters.push({t:o.type,x:cv.x,y:cv.y,l:.52,color:o.type==='NO RESPONSE'?'#91a0b5':'#ff9aa6'});return true;
  }

  function scoreInterview(cv,y){
    interviews++;combo++;bestCombo=Math.max(bestCombo,combo);motivationChange(9,'MOTIVATION +9');burst(cv.x,y,'#d9ff69',18);boom(combo>1?'INTERVIEW ×'+combo:'INTERVIEW!');cv.alive=false;
  }

  function update(dt,now){
    elapsed=(now-startAt)/1000;if(elapsed>=duration){endGame();return}
    motivationChange(-dt*.30);
    ...
    shotClock+=dt;spawnCV();if(elapsed>=nextPowerAt)spawnPower();if(elapsed>=nextChaosAt)changeChaos();powerups.forEach(p=>{p.x+=p.vx*dt;if(p.x<36||p.x>W-36)p.vx*=-1;p.life-=dt});
    ...
    for(const cv of cvs){
      ...
      if(cv.y<=tar.y+tar.h/2){if(Math.abs(cv.x-tar.x)<tar.width/2)scoreInterview(cv,tar.y);else{cv.alive=false;combo=0;motivationChange(-.06);floaters.push({t:'MISSED',x:cv.x,y:tar.y+24,l:.45,color:'#8191a6'})}}
    }
  ...
  function drawPower(p,now){
    ...
    if(lock){ctx.strokeStyle='rgba(124,243,173,.8)';ctx.lineWidth=1.5;const s=p.r+11;ctx.beginPath();ctx.moveTo(p.x-s,p.y-s/2);ctx.lineTo(p.x-s,p.y-s);ctx.lineTo(p.x-s/2,p.y-s);ctx.moveTo(p.x+s,p.y-s/2);ctx.lineTo(p.x+s,p.y-s);ctx.lineTo(p.x+s/2,p.y-s);ctx.stroke();ctx.fillStyle='#7cf3ad';ctx.font='950 8px system-ui';ctx.textAlign='center';ctx.fillText('TARGET',p.x,p.y-p.r-13)}
  ...
  function endGame(){
    ...
    $('rInterviews').textContent=interviews;$('rSent').textContent=sent;$('rCombo').textContent=bestCombo>1?'×'+bestCombo:'—';$('rBest').textContent=best;$('rMotivation').textContent=Math.round(motivation)+'%';
    $('resultLine').textContent=interviews?`${sent-interviews} CVs vanished, missed or got blocked. You finished on ${Math.round(motivation)}% motivation.`:`A very realistic run. You finished on ${Math.round(motivation)}% motivation.`;draw(performance.now());
  }
